...
const challengeMaxAmount = computed(() => {
  ...
  return Math.max(...dailyChallenge.value.levels.map((lv) => lv.maxSpending))
})
...
const challengeThresholds = computed(() => {
  if (!dailyChallenge.value || !challengeMaxAmount.value) return []
  return [...dailyChallenge.value.levels]
    .sort((a, b) => a.maxSpending - b.maxSpending)
    .map((lv) => ({
      maxSpending: lv.maxSpending,
      percent: (lv.maxSpending / challengeMaxAmount.value) * 100,
    }))
})
          ...
          <div ref="restSectionEl" class="flex flex-col gap-4">
            <AppCard>
              ...
              <p class="mb-4 text-xs leading-relaxed text-muted">
                매일 생활비를 아껴서 더 많은 코인을 받아보세요!
              </p>

              <div class="mb-5 flex items-end gap-2.5">
                <div
                  class="w-[138px] shrink-0 rounded-2xl border border-violet-200 bg-violet-50 p-3.5"
                >
                  <p class="mb-1 text-[11px] font-medium text-muted">오늘 사용 금액</p>
                  <p class="text-[26px] font-black leading-tight text-brand-primary">
                ...
                <div class="flex flex-1 gap-1.5">
                  ...
                  >
                    <div class="mb-1 flex h-[22px] items-center justify-center">
                      <span
                        v-if="lv.active"
                        class="whitespace-nowrap rounded-full bg-violet-100 px-2 py-0.5 text-[10px] font-extrabold text-brand-primary"
                        >현재 단계</span
                      >
                    </div>
                    <div
                      class="flex w-full flex-col items-center gap-1.5 rounded-2xl px-1 py-2.5"
                      :class="
                        lv.active
              ...
              <div class="pt-1">
                <div class="relative h-2.5 rounded-full bg-violet-100">
                  <div
                    class="absolute left-0 top-0 h-full rounded-full bg-gradient-to-r from-brand-primary to-violet-400"
                    :style="{ width: `${challengeSpentPercent}%` }"
                  />
                  <div
                    v-for="t in challengeThresholds"
                    :key="t.maxSpending"
                    class="absolute top-1/2 h-4 w-[3px] -translate-x-1/2 -translate-y-1/2 rounded bg-white/70"
                    :style="{ left: `${t.percent}%` }"
                  />
                  <div
                    class="absolute top-1/2 z-10 size-[22px] -translate-x-1/2 -translate-y-1/2 rounded-full border-[3.5px] border-white bg-brand-primary shadow-[0_2px_10px_rgba(124,58,237,0.5)]"
                    :style="{ left: `${challengeSpentPercent}%` }"
                  />
                </div>
                <div class="relative mt-2.5 h-7">
                  <span class="absolute left-0 text-[10px] font-medium text-muted">0원</span>
                  <span
                    v-for="t in challengeThresholds"
                    :key="t.maxSpending"
                    class="absolute -translate-x-1/2 whitespace-nowrap text-[10px] font-medium text-muted"
                    :style="{ left: `${t.percent}%` }"
                    >{{ t.maxSpending.toLocaleString() }}원</span
                  >
                  <span
                    class="absolute -translate-x-1/2 whitespace-nowrap text-[11px] font-extrabold text-brand-primary"
                    :style="{ left: `${challengeSpentPercent}%`, top: '14px' }"
                    >{{ dailyChallenge.todaySpending.toLocaleString() }}원</span
                  >
                </div>
              </div>
